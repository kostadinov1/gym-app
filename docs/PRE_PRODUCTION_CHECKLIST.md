...
- [ ] New Project → React Native → name it "Gym Tracker"
...
- [ ] Create **two separate apps** in RevenueCat (both with package `com.gencho.gymtracker`):
  - "Gym Tracker Dev" — for development builds
  - "Gym Tracker Prod" — for production builds
...
- [ ] Add app: Apps → Add app → Android → App not published yet → name "Gym Tracker" → package `com.gencho.gymtracker`
...
- [ ] Create app → Android → Package name `com.gencho.gymtracker`
